...
exports.deleteCourse = (courseCode) => {
    return new Promise((resolve, reject) => {
        // Delete related data in child tables first to avoid FK violations
        // Execute sequentially since mysql2 createConnection doesn't support multiple statements
        const deleteHandbookSlotCourse = `DELETE FROM handbook_slot_course WHERE courseCode = ?`;
        db.query(deleteHandbookSlotCourse, [courseCode], (err) => {
            if (err) return reject(err);

            const deletePrereq = `DELETE FROM prerequisite WHERE courseCode = ? OR prerequisiteCourseCode = ?`;
            db.query(deletePrereq, [courseCode, courseCode], (err) => {
                if (err) return reject(err);

                const deleteSelectedCourse = `DELETE FROM selected_course WHERE courseCode = ?`;
                db.query(deleteSelectedCourse, [courseCode], (err) => {
                    if (err) return reject(err);

                    const deletePatternDetail = `DELETE FROM pattern_detail WHERE courseCode = ?`;
                    db.query(deletePatternDetail, [courseCode], (err) => {
                        if (err) return reject(err);

                        const deleteFilteredPatternDetail = `DELETE FROM filtered_pattern_detail WHERE courseCode = ?`;
                        db.query(deleteFilteredPatternDetail, [courseCode], (err) => {
                            if (err) return reject(err);

                            const deleteFreeElective = `DELETE FROM free_elective_offering WHERE courseCode = ?`;
                            db.query(deleteFreeElective, [courseCode], (err) => {
                                if (err) return reject(err);

                                const deleteCourseRegStatus = `DELETE FROM course_registration_status WHERE courseCode = ?`;
                                db.query(deleteCourseRegStatus, [courseCode], (err) => {
                                    if (err) return reject(err);

                                    const deleteLecturerPref = `DELETE FROM lecturer_preference WHERE courseCode = ?`;
                                    db.query(deleteLecturerPref, [courseCode], (err) => {
                                        if (err) return reject(err);

                                        // Delete sections
                                        const deleteSections = `DELETE FROM section WHERE courseCode = ?`;
                                        db.query(deleteSections, [courseCode], (err) => {
                                            if (err) return reject(err);

                                            // Delete the course itself
                                            const deleteCourseQuery = `DELETE FROM course WHERE courseCode = ?`;
                                            db.query(deleteCourseQuery, [courseCode], (err, result) => {
                                                if (err) return reject(err);

                                                resolve({
                                                    deleted: result.affectedRows > 0,
                                                    message: result.affectedRows > 0 ? 'Course and all related data deleted successfully' : 'Course not found',
                                                    affectedRows: result.affectedRows
                                                });
                                            });
                                        });
...
// Delete a section and all its related data
exports.deleteSection = (sectionID) => {
    return new Promise((resolve, reject) => {
        // Delete related data in child tables first
        const deletePatternDetail = `DELETE FROM pattern_detail WHERE sectionID = ?`;
        db.query(deletePatternDetail, [sectionID], (err) => {
            if (err) return reject(err);

            const deleteFilteredPatternDetail = `DELETE FROM filtered_pattern_detail WHERE sectionID = ?`;
            db.query(deleteFilteredPatternDetail, [sectionID], (err) => {
                if (err) return reject(err);

                // Delete the section itself
                const deleteSectionQuery = `DELETE FROM section WHERE sectionID = ?`;
                db.query(deleteSectionQuery, [sectionID], (err, result) => {
                    if (err) return reject(err);

                    resolve({
                        deleted: result.affectedRows > 0,
                        message: result.affectedRows > 0 ? 'Section and all related data deleted successfully' : 'Section not found',
                        affectedRows: result.affectedRows
                    });
                });
            });
